  ...
  export default function Home() {
    const [result, setResult] = useState<null | { text: string; accuracy: string; category: string }>(null);
    ...
    const handlePrediction = async () => {
      ...
      console.log(data);
      const geminiOutput = data?.candidates?.[0]?.content?.parts?.[0]?.text || "Unknown";
    ...
    return (
      <div
        className={`min-h-screen py-12 px-4 ${darkMode ? "bg-zinc-900 text-white" : "bg-white text-black"} transition-colors`}
      >
